...
# import sys
# sys.path
# sys.path.append('/mnt/lustre/sjtu/home/bhl66/EEND_PyTorch')
# print(sys.path)
...
def train(args):
    ...
    # Supporting all parameter for checkpoint. (Reference: Pytorch document)
    init_epoch = 1
    if args.initmodel:
        checkpoint = torch.load(args.initmodel)
        model.load_state_dict(checkpoint['model_state_dict'])
        optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        scheduler.load_state_dict(checkpoint['scheduler_state_dict'])
        init_epoch = checkpoint['epoch']
        # generator = torch.Generator().set_state(checkpoint['train_loader_generator'])
        train_iter = DataLoader(
            train_set,
            batch_size=args.batchsize,
            shuffle=True,
    ...
    for epoch in range(init_epoch, args.max_epochs + 1):
        ...
        # save all points support checkpoint training
        assert isinstance(train_iter.sampler, RandomSampler)
        torch.save({
